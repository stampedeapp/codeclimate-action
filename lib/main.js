...
function run(downloadUrl = DOWNLOAD_URL, executable = EXECUTABLE, coverageCommand = DEFAULT_COVERAGE_COMMAND, workingDirectory = DEFAULT_WORKING_DIRECTORY, codeClimateDebug = DEFAULT_CODECLIMATE_DEBUG, coverageLocationsParam = DEFAULT_COVERAGE_LOCATIONS, coveragePrefix, verifyDownload = DEFAULT_VERIFY_DOWNLOAD) {
    return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
        if ((0, os_1.platform)() === 'win32') {
            ...
            (0, core_1.error)('🚨 CodeClimate Reporter will not run on Windows!');
            return reject(err);
        }
        let lastExitCode = 1;
        if (workingDirectory) {
            ...
            try {
                ...
                (0, core_1.error)(err.message);
                (0, core_1.error)('🚨 Changing working directory failed!');
                return reject(err);
            }
        }
        try {
            ...
            (0, core_1.error)(err.message);
            (0, core_1.error)('🚨 CC Reporter download failed!');
        ...
        if (verifyDownload === 'true') {
            ...
            try {
                ...
                (0, core_1.error)(err.message);
                (0, core_1.error)('🚨 CC Reporter checksum verfication failed!');
                return reject(err);
            }
            try {
                ...
                (0, core_1.error)(err.message);
                (0, core_1.error)('🚨 CC Reporter GPG signature verfication failed!');
                return reject(err);
            }
        }
        const execOpts = {
        ...
        try {
            ...
            (0, core_1.error)(err.message);
            (0, core_1.error)('🚨 CC Reporter before-build checkin failed!');
            return reject(err);
        }
        if (coverageCommand) {
            try {
                ...
                (0, core_1.error)(err.message);
                (0, core_1.error)('🚨 Coverage run failed!');
                return reject(err);
            }
        }
        else {
        ...
        if (coverageLocations.length > 0) {
            ...
            for (const i in coverageLocations) {
                ...
                if (!type) {
                    ...
                    (0, core_1.error)(err.message);
                    (0, core_1.error)('🚨 Coverage formatter type not set! Each coverage location should be of the format <file_path>:<coverage_format>');
                    return reject(err);
                }
                const commands = [
                    'format-coverage',
                ...
                try {
                    ...
                    (0, core_1.error)(err.message);
                    (0, core_1.error)('🚨 CC Reporter coverage formatting failed!');
                    return reject(err);
                }
            }
            // Run sum coverage.
            ...
            try {
                ...
                (0, core_1.error)(err.message);
                (0, core_1.error)('🚨 CC Reporter coverage sum failed!');
                return reject(err);
            }
            ...
            try {
                ...
                (0, core_1.error)(err.message);
                (0, core_1.error)('🚨 CC Reporter coverage upload failed!');
                return resolve();
            }
        }
        try {
            ...
            (0, core_1.error)(err.message);
            (0, core_1.error)('🚨 CC Reporter after-build checkin failed!');
            return reject(err);
        }
    }));
}
...
if (require.main === module) {
    ...
    run(DOWNLOAD_URL, EXECUTABLE, coverageCommand, workingDirectory, codeClimateDebug, coverageLocations, coveragePrefix, verifyDownload).catch(err => {
        process.exit(0);
    });
}
